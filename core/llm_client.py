    ...
    def chat_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            if retry_attempt > 0:
                logger.warning(
                    f"LLM API 重试第 {retry_attempt} 次 - Provider: {self.provider}, Model: {self.model}, Agent: {agent_name}"
                )
            ...
            if choice.message.tool_calls:
                ...
                # 记录工具调用（简化版）
                logger.debug(
                    f"[{agent_name}] 工具调用: {[call['function'] for call in tool_calls_info]}"
                )
            ...
            # 简化API调用记录，不记录详细对话
            logger.debug(
                f"[{agent_name}] LLM响应长度: {len(content) if content else 0}字符"
            )
